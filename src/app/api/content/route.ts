...
export function GET(request: NextRequest) {
  ...
  if (session.role === 'platform-admin') {
    return NextResponse.json({ tenantId: session.tenantId, websiteContent: null });
  }

  return NextResponse.json({ tenantId: session.tenantId, websiteContent: inMemoryPlatformRepository.getWebsiteContent(session.tenantId) });
}