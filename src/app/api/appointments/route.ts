...
export function GET(request: NextRequest) {
  ...
  if (session.role === 'platform-admin') {
    return NextResponse.json({ tenantId: session.tenantId, appointments: [] });
  }

  return NextResponse.json({ tenantId: session.tenantId, appointments: inMemoryPlatformRepository.listAppointments(session.tenantId) });
}